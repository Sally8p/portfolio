document.addEventListener("DOMContentLoaded", () => {
  ...
  function startGame() {
    ...
    gameContent.style.display = "flex"; // 게임 내용 보이기 (모바일 뷰포트)
    resetGame(); // 게임 초기화
  }
  ...
  function moveSnake() {
    ...
    if (head.x === food.x && head.y === food.y) {
      ...
      if (score % speedIncreaseInterval === 0) {
        speed -= speedIncreaseAmount; // 먹이를 5개 먹을 때마다 속도를 증가시킴 (속도 감소)
      }
      spawnFood(); // 새로운 음식 생성
    } else {
      snake.pop(); // 꼬리 제거
  ...
  function updateScoreHistory() {
    scoreHistory.innerHTML = scoreRecords
      .map(
        (score, index) => `<div>${index + 1}. 먹은 갯수: ${score}개 🍫</div>`
      )
      .join("");
  }
  ...
  // 다시 시작 버튼과 결과 보기 버튼 클릭 이벤트 리스너 추가
